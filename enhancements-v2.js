(()=>{
  ...
  const css=`
.lm-filter-wrap{display:flex;flex-direction:column;gap:10px;margin-bottom:22px}
...
.lm-quick-status{margin-top:8px;font-size:11px;color:#7e8b98}
...
`;
  ...
  function buildFilterUI(){
    const old=document.querySelector('.lm-filter-wrap');if(old)old.remove();
    ...
    const cats=['全部',...state.categories.map(c=>c.name)];
    cats.forEach(name=>{const b=document.createElement('button');b.type='button';b.className='lm-filter-btn'+(state.category===name?' active':'');b.textContent=name;b.dataset.category=name;b.onclick=()=>{state.category=name;buildFilterUI();applyDomFilters()};catLine.appendChild(b)});
    ...
    if(state.role!=='sales'){
      ...
      ['全部','已发布','草稿'].forEach(name=>{const b=document.createElement('button');b.type='button';b.className='lm-filter-btn'+(state.status===name?' active':'');b.textContent=name;b.dataset.status=name;b.onclick=()=>{state.status=name;buildFilterUI();applyDomFilters()};statusLine.appendChild(b)});
  ...
  async function queryProducts(){
    ...
    if(term)q=q.or(`name.ilike.%${term}%,sku.ilike.%${term}%,description.ilike.%${term}%`);
  ...
  async function lmTogglePublish(id){
    ...
    if(typeof window.enhanceProductFilters==='function')window.enhanceProductFilters();
  }
  ...
  function addDetailCopyButton(){
    ...
    const anyButton=detail.querySelector('.card-actions button[onclick*="copyProductSpecs"]');
    if(!id&&anyButton){const m=(anyButton.getAttribute('onclick')||'').match(/copyProductSpecs\\('([^']+)'\\)/);if(m)window.__lmLastDetailId=m[1]}
    const pid=id||window.__lmLastDetailId;if(!pid)return;
    const box=anyButton?.parentElement||detail.querySelector('.detail>div:nth-child(2)');if(!box)return;
  ...
  function patchCard(){
    ...
    const wrapped=function(p){
      ...
      return html.replace('<div class="card-actions">','<div class="card-actions"><button class="'+klass+'" onclick="lmTogglePublish(\''+p.id+'\')">'+action+'</button>');
    };
  ...
  function patchShell(){
    ...
    const wrapped=function(content){
      ...
      setTimeout(async()=>{
        patchCard();
        if(window.view==='products')await enhanceProductFilters();
        if(window.view==='detail')addDetailCopyButton();
      },0);
    };
